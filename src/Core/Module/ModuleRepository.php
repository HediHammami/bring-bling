       ...
       private function getModuleAttributes(string $moduleName, bool $isValid): array
{
    ...
    if ($tmpModule !== null) {
        ...
        $attributes['parent_class'] = is_object($tmpModule) ? get_parent_class($tmpModule) : null;
        $attributes['is_paymentModule'] = is_object($tmpModule) ? is_subclass_of($tmpModule, 'PaymentModule') : false;
        $attributes['is_configurable'] = is_object($tmpModule) ? method_exists($tmpModule, 'getContent') : false;
    }
   ...
   private function getModuleDiskAttributes(string $moduleName, bool $isValid, int $filemtime): array
{
    ...
    $attributes = [
        ...
        'is_valid' => $isValid,
        'version' => null, // Default to null if $isValid is false
        'path' => $path,
    ];

    if ($isValid) {
        try {
            $moduleInstance = ModuleLegacy::getInstanceByName($moduleName);
            $attributes['version'] = $moduleInstance->version;
        } catch (Throwable $e) {
