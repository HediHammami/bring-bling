...
<div class="product-container{if !empty($settings.price)} has-price{/if}{if !empty($settings.stock)} has-stock{/if}{if $has_buttons} has-buttons{/if}">
			...
			<a class="product_img_link"	href="{$product.link|escape:'html':'UTF-8'}">
				<img
					...
					{if !empty($image_sizes[$img_type])}
						...
						height="{$image_sizes[$img_type]['height']|intval}"
					{/if}
					{if !empty($product.second_img_src)}
						data-toggle-src="{$product.second_img_src|escape:'html':'UTF-8'}"
					{/if}
					loading="lazy"
				/>
			</a>
	...
	{if !empty($settings.product_man) && $product.id_manufacturer && $product.man_name && !empty($product.man_url)}
		<div class="prop-line product-manufacturer">
			...
			{if !empty($product.man_img_src)}
				...
				<img src="{$product.man_img_src|escape:'html':'UTF-8'}" class="product-manufacturer-img"{if !empty($image_sizes[$m_img_type])} width="{$image_sizes[$m_img_type]['width']|intval}" height="{$image_sizes[$m_img_type]['height']|intval}"{/if} loading="lazy">
			{else}
