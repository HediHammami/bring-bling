...
<div id="{$carousel.identifier|escape:'html':'UTF-8'}" class="easycarousel {$tpl_settings.custom_class|escape:'html':'UTF-8'}{if $in_tabs} ec-tab-pane{else} carousel_block{/if}{if !empty($custom_class)} {$custom_class|escape:'html':'UTF-8'}{/if}{if empty($carousel.name) && $c_settings.n} nav_without_name{/if}{if !$ec_is_mobile}{if $c_settings.n == 2} n-hover{/if}{if $c_settings.p == 2} p-hover{/if}{/if}">
	...
	<div class="block_content">
		<div class="c_container {if $c_settings.type == 1}ecarousel{if $carousel.is_swiper} swiper{/if}{else if $c_settings.type == 2}scroll-x{else}ec-grid{/if} xl-{$c_settings.i|intval} lg-{$c_settings.i_1200|intval} md-{$c_settings.i_992|intval} sm-{$c_settings.i_768|intval} xs-{$c_settings.i_480|intval}" data-settings="{$c_settings|json_encode|escape:'html':'UTF-8'}"{if !empty($tpl_settings.second_image)} data-second-img="1"{/if}>
			...
			{/foreach}
			{if $carousel.is_swiper}</div>{/if}
		</div>
		{if $c_settings.type == 1 && !empty($c_settings.p)}<div class="sw-pgn"></div>{/if}
	</div>
