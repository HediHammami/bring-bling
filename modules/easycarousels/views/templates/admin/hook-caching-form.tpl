...
<div class="panel clearfix">
	...
	{if !empty($no_caching_warning)}
		<div class="alert alert-warning nomargin">{$no_caching_warning|escape:'html':'UTF-8'}</div>
	{else}
	...
	<div class="form-group col-lg-12 main-caching-option">
		...
		<div class="col-lg-2">
			<select class="caching-options switch-select{if $settings.time} yes{/if}" name="settings[time]">
				...
				<option value="3600"{if $settings.time == 3600} selected{/if}>{l s='Yes - reset cache every hour' mod='easycarousels'}</option>
				<option value="21600"{if $settings.time == 21600} selected{/if}>{l s='Yes - reset cache every 6 hours' mod='easycarousels'}</option>
				<option value="43200"{if $settings.time == 43200} selected{/if}>{l s='Yes - reset cache every 12 hours' mod='easycarousels'}</option>
				<option value="86400"{if $settings.time == 86400} selected{/if}>{l s='Yes - reset cache daily (24h)' mod='easycarousels'}</option>
				<option value="604800"{if $settings.time == 604800} selected{/if}>{l s='Yes - reset cache weekly (168h)' mod='easycarousels'}</option>
	...
	<input type="hidden" name="settings[category]" value="{$settings.category|intval}">
