...
require_once(_PS_MODULE_DIR_ . 'pragmaproductsfacilities/classes/Productsfacilities.php');
...
class Cart extends CartCore
{
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null, $fullInfos = true, bool $keepOrderPrices = false)
    {
        ...
        /** FBR */
        // Build SELECT
        $sql->select('cp.`id_product_attribute`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, cp.`id_customization`, cp.`number_payment`, pl.`name`, p.`is_virtual`,
    ...
    /** FBR */
    public static function updateNumberPayment($id_cart, $id_product, $id_customization, $id_product_attribute, $number_payment)
    {
        Db::getInstance()->execute(
            'UPDATE `' . _DB_PREFIX_ . 'cart_product`
                SET `number_payment` = ' . $number_payment . '
    ...
    /** FBR */
    public static function getNumberPayment($id_cart, $id_product, $id_customization, $id_product_attribute)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT number_payment FROM ' . _DB_PREFIX_ . 'cart_product 
                WHERE `id_product` = ' . (int) $id_product . ' 
                AND `id_customization` = ' . (int) $id_customization . ' 
                AND `id_product_attribute` = ' . (int) $id_product_attribute . ' 
                AND `id_cart` = ' . (int) $id_cart
