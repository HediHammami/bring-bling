...
class AmazzingFilter extends Module
{
    ...
    public function oos($action, $params = [])
    {
        ...
        switch ($action) {
            ...
            case 'allowedQuery':
                ...
                break;
            case 'stockShopAssociation':
                $shop_group = Shop::getGroupFromShop($params['id_shop'], false);
                $ret = $shop_group['share_stock'] ? 'sa.id_shop_group = ' . (int) $shop_group['id']
                    : 'sa.id_shop = ' . (int) $params['id_shop'];
                break;
        }
            ...
            case 'indexation':
                ...
                foreach ($check_values as $key => $data) {
                    if (!isset($values[$key])) {
                        ...
                        foreach ($data as $d) {
                            if (!$values[$key]) {
                                $values[$key] = (int) $this->db->getValue('
    ...
    public function getCombinationImages($combination_ids, $id_lang)
    {
        $combination_images_data = $this->db->executeS('
            ...
            WHERE pai.id_product_attribute IN (' . $this->sqlIDs($combination_ids) . ')
